...
<BODY>
    ...
    <div class="trademark">
      <img src="img/mark4.PNG" width="600" height="600" />
    </div>
    <div>
      <!-- 상품 목록 화면 -->
   	<% JpDAO jpDAO = new JpDAO();
    	ResultSet rs;
    	int i = 0, k = 0;
        int rows = jpDAO.GetJpcount();
    	int count = 0;
    	int index = 0;
    	rs = jpDAO.BringJpData();            // 데이터 베이스에서 board 테이블에 저장된 데이터를 가져옴
    	
    	String[] jpid = new String[rows];
        String[] jpName = new String[rows];
        String[] jpPrice = new String[rows];
        String[] jpPicture = new String[rows];
        
        while (rs.next())
        {
        	jpid[i] = rs.getString("id");
            jpName[i] = rs.getString("name");
            jpPrice[i] = rs.getString("price");
            jpPicture[i] = rs.getString("img");
            i++;
        }
  	%>
      ...
      <colgroup>
		...
		<thead>
			...
			<tr>
			<% for(int j=0;j<2;j++) {%>
				<td class="textcontent"><a href="./jpform.jsp?index=<%=jpid[a] %>&tag=jp"><%=jpName[a] %><br><%=jpPrice[a] %>원 </a></td>
				<td class="imgcontent"><a href="./jpform.jsp?index=<%=jpid[a] %>&tag=jp"><img src="<%=jpPicture[a] %>" width="320" height="200"/></a></td>
			<%
			a++;
			}%>
			</tr>
